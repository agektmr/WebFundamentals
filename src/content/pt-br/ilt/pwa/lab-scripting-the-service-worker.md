...
**Optional**: Open the site on an [unsupported browser](https://jakearchibald.github.io/isserviceworkerready/) and verify that the support check conditional works.
...
Save the file and refresh the page. Look at the logs in the console; notice that the new service worker installs but does not activate. In Chrome, you can see the waiting service worker in the **Application** tab in DevTools.
...
[Unregister the service worker](tools-for-pwa-developers#unregister) and refresh the page. Click **Other page**, **Another page** and **Back**.
